...
# Ollama Cloud Models - run on Ollama's servers, no local GPU needed
# Requires: ollama signin (Ollama v0.12+)
OLLAMA_CLOUD_MODELS = [
    ("gpt-oss:20b-cloud", "20B", "GPT-OSS 20B Cloud - Good quality, fast"),
    ("gpt-oss:120b-cloud", "120B", "GPT-OSS 120B Cloud - Excellent quality"),
    ("qwen3-coder:480b-cloud", "480B", "Qwen3 Coder 480B Cloud - Best for technical"),
    ("deepseek-v3.1:671b-cloud", "671B", "DeepSeek V3.1 671B Cloud - Maximum quality"),
]
...
def get_detected_vram_info() -> str:
    ...
    return "⚠️ GPU VRAM could not be detected. Please select manually or use ☁️ Cloud models."
...
def get_vram_capacity_info(vram_gb: int, model_name: str = None) -> str:
    ...
    if model_name and model_name.endswith("-cloud"):
        return f"☁️ **Cloud model** - No local GPU needed | Requires `ollama signin`"
...
def get_model_choices(vram_gb: int) -> List[Tuple[str, str]]:
    """Returns model choices - INSTALLED models first, then others, then cloud."""
    ...
    for model_name in installed:
        ...
        if model_info:
            ...
            # Find in cloud models
            for cloud_name, cloud_size, cloud_desc in OLLAMA_CLOUD_MODELS:
                if cloud_name == model_name:
                    desc = cloud_desc
                    size = cloud_size
                    break
            else:
        ...
        if is_cloud:
            label = f"☁️💾 {model_name} ({size}) [INSTALLED] - {desc}"
        else:
    ...
    # THIRD: Add cloud models (always available, no VRAM needed)
    for cloud_name, cloud_size, cloud_desc in OLLAMA_CLOUD_MODELS:
        if cloud_name not in seen_models:
            label = f"☁️ {cloud_name} ({cloud_size}) - {cloud_desc}"
            choices.append((label, cloud_name))
            seen_models.add(cloud_name)
...
def pull_ollama_model(model_name: str, force_update: bool = False, progress=gr.Progress()):
    ...
    if model_name.endswith("-cloud"):
        return f"""☁️ **Cloud model selected: {model_name}**

Cloud models run on Ollama's servers - no local GPU needed!

**Setup required:**
1. Update Ollama to v0.12+: https://ollama.com/download
2. Sign in: `ollama signin`
3. Pull model: `ollama pull {model_name}`

**Note:** Cloud models require internet and send data to Ollama servers.
For complete privacy, use local models instead."""
...
def create_gradio_app():
    ...
    with gr.Blocks(
        ...
        Supports **OpenAI GPT-4**, **local Ollama models**, and **☁️ Ollama Cloud models**.
        ...
        with gr.Tabs():
                    ...
                    with gr.Column(scale=1):
                        ...
                        with gr.Group(visible=True) as ollama_group:
                            ...
                            ollama_model_select = gr.Dropdown(
                                ...
                                value=initial_model,
                                label="Ollama Model (local + ☁️ cloud)",
                            )
            ...
            with gr.TabItem("📊 VRAM Guide"):
                ...
                | **No GPU** | ☁️ `gpt-oss:120b-cloud` | **Cloud - no GPU needed!** |
                
                **Note:** ✅ = Fits comfortably, ⚠️ = Works, but tight, ☁️ = Cloud (no local GPU)
                """)
            ...
            with gr.TabItem("❓ Help"):
                ...
                ### ☁️ Ollama Cloud (No GPU needed!)
                
                For users without GPU or with low VRAM:
                
                1. **Update Ollama** to v0.12+: https://ollama.com/download
                2. **Sign in:** `ollama signin`
                3. **Select a ☁️ cloud model** from the dropdown
                4. **Pull it:** `ollama pull gpt-oss:120b-cloud`
                
                **Available cloud models:**
                - `gpt-oss:20b-cloud` - Good quality, fast
                - `gpt-oss:120b-cloud` - Excellent quality
                - `qwen3-coder:480b-cloud` - Best for technical texts
                - `deepseek-v3.1:671b-cloud` - Maximum quality
                
                ⚠️ **Note:** Cloud models send data to Ollama servers.
                ...
                | "Cloud model not working" | Run `ollama signin` |
                | "PDF compilation failed" | Install LaTeX (TeX Live / MiKTeX) |
                | "Out of Memory" | Choose smaller model or use ☁️ cloud |
                """)
